...
export default function NavPatternTertiary(props) {
  ...
  async function calculateWidths() {
    if (components.filter((item) => item.sub_type === "tertiary").length) {
      ...
      if (activeCase === "visible") {
        let contentArr = fullNav.slice(0, 4);
  ...
  const handleCaseSelect = (event) => {
    ...
    setActiveCase(value);
    // update history
    history.push(`/patterns/tertiary-navigation?case=${value}`);
  };
...
NavPatternTertiary.defaultProps = {
  id: 1,
  name: "Browse"
};
